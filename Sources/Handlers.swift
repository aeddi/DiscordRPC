...
extension Session {
    ...
    ///     - a reference to the related `Session` instance
    ///     - an `EventReady`
    public func onConnect(
        handler: @escaping (_ session: Session, _ eventReady: EventReady) -> Void
    ) {
    ...
    public func onDisconnect(
        handler: @escaping (_ session: Session, _ closeError: EventClose) -> Void
    ) {
    ...
    public func onResponse(
        handler: @escaping (_ session: Session, _ nonce: String, _ commandType: CommandType, _ response: Data) -> Void
    ) {
    ...
    ///     - an `EventError`
    public func onError(
        handler: @escaping (_ session: Session, _ nonce: String, _ eventError: EventError) -> Void
    ) {
    ...
    public func onEvent(
        handler: @escaping (_ session: Session, _ eventType: EventType, _ event: Data) -> Void
    ) {
